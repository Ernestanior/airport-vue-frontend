<template>
  <div class="item_container">
    <div v-if="order_history.length==0" class="empty">
      <img class="icon_empty" src="@/assets/icon/jieji/empty_3.png"></img>
      <div class="text_empty">It`s Empty</div>
    </div>
    <div v-else>
      <a class="order_content" v-for="item in order_history" :key="item.order_id" @click="toDetail(item)">
...
<script>
  ...
  export default {
    ...
    created() {
      this.$loading.show()
      getDoneOrder(this.$store.state.user.email).then(res => {
        // console.log(res);
        if (res) { this.order_history = res.data.orders }
        this.$loading.hide()
      })
    },
  }
